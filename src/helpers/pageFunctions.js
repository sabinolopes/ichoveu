...
const createButton = (url) => {
  ...
  button.addEventListener('click', async () => {
    ...
    const weekCondition = forecastday.map(({ date, day }) => ({
      date,
...
export function createCityElement(cityInfo) {
  ...
  cityElement.appendChild(createButton(apiUrl));
...
const appendLi = (cityInfo) => {
  const ul = document.querySelector('#cities');

  ul.appendChild(createCityElement(cityInfo));
};
...
export async function handleSearch(event) {
  ...
  const promises = foundCities.map(({ url }) => getWeatherByCity(url));

  Promise.all(promises)
    .then((cities) => cities.forEach((city) => appendLi(city)))
